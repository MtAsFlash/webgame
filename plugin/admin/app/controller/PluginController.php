...
class PluginController extends Base
{
     ...
     * @var string[]
     */
    protected $noNeedAuth = ['schema', 'captcha', 'login'];
    ...
    public function list(Request $request): Response
    {
        ...
        $response         = $client->get('/plugin_list.json', ['query' => $query]);
        ...
        foreach ($data['data'] as $key => $item) {
            $name                            = $item['name'];
            $data['data'][$key]['installed'] = $installed[$name] ?? 0;
            $data['data'][$key]['disabled']  = $disabled;
        }
    ...
    public function install(Request $request): Response
    {
        ...
        // 下载zip文件
        $dataUrl    = config('plugin.admin.app.plugin_market_host')."/plugins/".$name."-v".$version.".zip";
        $base_path  = base_path()."/plugin/$name";
        $zip_file   = "$base_path.zip";
        $extract_to = base_path().'/plugin/';
        $this->downloadZipFile($dataUrl, $zip_file);
    ...
    protected function findCmd(string $name, string $default = null, array $extraDirs = [])
    {
        ...
        if ('\\' === DIRECTORY_SEPARATOR) {
            ...
            $suffixes = array_merge($pathExt ? explode(PATH_SEPARATOR, $pathExt) : ['.exe', '.bat', '.cmd', '.com'],
                $suffixes);
        }
