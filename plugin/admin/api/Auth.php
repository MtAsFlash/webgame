...
class Auth
{
    ...
    public static function canAccess(string $controller, string $action, int &$code = 0, string &$msg = ''): bool
    {
        ...
        // 查询是否有当前控制器的规则
        /*$rule = Rule::where(function ($query) use ($controller, $action) {
            $query->where('key', "$controller@$action")->whereOr('key', $controller);
        })->whereIn('id', $rule_ids)->find();*/
        $ruleId = Rule::where(function ($query) use ($controller, $action) {
            $query->where('key', "$controller@$action");
        })->value('id');

        if (empty($ruleId) || ! in_array($ruleId, $rule_ids)) {
            $msg  = '无权限';
            $code = 2;
